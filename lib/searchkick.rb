...
module Searchkick
  ...
  class << self
    attr_accessor :search_method_name, :wordnet_path, :timeout, :models, :client_options, :redis, :index_prefix, :index_suffix, :queue_name, :model_options, :process_queue_name
  ...
  self.process_queue_name = :searchkick
  self.model_options = {}
  ...
  def self.load_records(records, ids)
    records =
      if records.respond_to?(:primary_key)
        # ActiveRecord
        records.with_deleted.where(records.primary_key => ids) if records.primary_key
